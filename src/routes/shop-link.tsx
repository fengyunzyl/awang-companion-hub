...
function ShopLink() {
  ...
  const [manual, setManual] = useState<ManualInfo | null>(() => {
    if (typeof window === "undefined") return null;
    const raw = localStorage.getItem("aw_shop_manual");
    if (!raw) return null;
    try {
      return JSON.parse(raw) as ManualInfo;
    } catch {
      return null;
    }
  });
  const [dialogOpen, setDialogOpen] = useState(false);
  const [form, setForm] = useState<ManualInfo>(EMPTY);
    ...
    <div className="min-h-screen flex flex-col" style={{ background: "#fafafa" }}>
      ...
      <div className="flex-1 flex flex-col px-6 pt-6 pb-10">
        ...
        <button
          onClick={save}
          disabled={!canSubmit}
          className="bg-primary text-primary-foreground rounded-full font-semibold text-base shadow-button mt-6 disabled:opacity-50"
          style={{ height: 52 }}
        >
          保存
        </button>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-[340px] rounded-2xl !slide-in-from-left-0 !slide-in-from-top-0 !slide-out-to-left-0 !slide-out-to-top-0">
          ...
          <DialogFooter className="flex-row gap-2">
            ...
            <Button className="flex-1" onClick={saveManual} disabled={!canSave}>
              保存
            </Button>
          </DialogFooter>
        </DialogContent>
